...
## Previously Removed Plugins (Precedent)

These were specifically removed by Jagex request or RuneLite policy — do not recreate their functionality:

- AoE (Area of Effect) plugin
- In-game Zulrah helper
- Volcanic Mine helper
- Demonic Gorilla plugin
- Fight Cave (Jad) plugin — "trivialises the fight"
- Cerberus plugin
- Dynamic Puro-Puro spawn highlighting

## Other Rejected Features

- Camera position presets (yaw/pitch/position)
- Auto-rejoin parties (undue strain on party servers)
- Hiscores for personal bests (data easily spoofed)
- AFK agility training aids
- Highlighting offline friends (privacy concern)
